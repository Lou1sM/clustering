...
if __name__ == "__main__": # Necessary to make multiproc work
    ...
    new_ae = functools.partial(nns.make_ae,NZ=ARGS.NZ,image_size=ARGS.image_size,num_channels=ARGS.num_channels)
    # Put one copy of the dataset on each gpu being used
    gpus = [torch.device(f'cuda:{i}') for i in ARGS.gpus]
    dsets = [vision_dsets.get_vision_dset(ARGS.dset,gpu) for gpu in gpus]
    filled_pretrain = functools.partial(multi_ae.rtrain_ae,args=ARGS,should_change=True)
    filled_label = functools.partial(multi_ae.label_single,args=ARGS,umap_abl=False)
    ...
    if 1 in ARGS.sections: # Train aes on reconstruction loss
        pretrain_start_time = time()
        aes = []
        print(f"Instantiating {len(aeids)} aes...")
        tiled_dsets = np.tile(dsets,len(aeids))
        for i,aeid in enumerate(aeids):
            dset_idx = i%len(ARGS.gpus)
            dset = dsets[dset_idx]
            aes.append({'aeid':aeid, 'ae':new_ae(aeid,device=dset.device),'dset':dset})
    ...
    else: # Load pretrained aes
        print(f"Loading {len(aeids)} aes...")
        ae_dicts_without_aes = []
        for i,aeid in enumerate(aeids):
            dset_idx = i%len(ARGS.gpus)
            dset = dsets[dset_idx]
            ae_dicts_without_aes.append({'aeid':aeid,'dset':dset})
        aes = utils.apply_maybe_multiproc(filled_load_ae,ae_dicts_without_aes,split=ARGS.split,single=ARGS.single)
    ...
    try: gt_labels = np.load(os.path.join(labels_dir,'gt_labels.npy'))
        ...
        d = vision_dsets.get_vision_dset(ARGS.dset,device=f'cuda:{ARGS.gpus[0]}')
    ...
    if 3 in ARGS.sections:
        ...
        centroids_by_id, ensemble_labels, all_agree = multi_ae.build_ensemble(vecs_and_labels,ARGS,pivot=gt_labels)
        ...
        for meta_epoch_num in range(ARGS.max_meta_epochs):
            ...
            for aedict in copied_aes:
                if not hasattr(aedict['ae'],'pred'):
                    ...
                    aedict['ae'].pred = nns.mlp(ARGS.NZ,25,nout,device=aedict['ae'].device)
            ...
            if ARGS.ablation == 'none':
                filled_train = functools.partial(multi_ae.train_ae,args=ARGS,targets=ensemble_labels,all_agree=all_agree,sharing_ablation=False)
            elif ARGS.ablation == 'sharing':
                filled_train = functools.partial(multi_ae.train_ae,args=ARGS,targets=labels,all_agree=np.ones(ARGS.dset_size).astype(np.bool),sharing_ablation=True)
            elif ARGS.ablation == 'filtering':
                filled_train = functools.partial(multi_ae.train_ae,args=ARGS,targets=ensemble_labels,all_agree=np.ones(ARGS.dset_size).astype(np.bool),sharing_ablation=False)
            print(f"Training aes {list(aeids)}...")
