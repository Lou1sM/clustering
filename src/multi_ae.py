...
def pretrain_ae(args,dset,ae):
    ...
    if args.pretrained: pass
        ...
        torch.save({'enc':ae.enc,'dec':ae.dec},f'../checkpoints/pt{ae.identifier}.pt')

def build_ensemble_gt(ensemble_results):
    ...
    usable_mid_labels = {aeid:result['mid_labels'] for aeid,result in ensemble_results.items() if get_num_labels(result['mid_labels']) == ensemble_num_labels}
    usable_latent_labels = {aeid:result['latent_labels'] for aeid,result in ensemble_results.items() if get_num_labels(result['latent_labels']) == ensemble_num_labels}
    ...
    for aeid in usable_mid_labels.keys():
        mid_centroids = np.stack([ensemble_results[aeid]['mids'][(ensemble_labels==i)*all_agree].mean(axis=0) for i in sorted(set(ensemble_labels))])
        mid_centroids_by_id[aeid] = mid_centroids
        np.save(f'mid_centroids{aeid}.npy', mid_centroids)
    for aeid,latent in usable_latent_labels.items():
        latent_centroids = np.stack([ensemble_results[aeid]['latents'][(ensemble_labels==i)*all_agree].mean(axis=0) for i in sorted(set(ensemble_labels))])
        latent_centroids_by_id[aeid] = latent_centroids
...
def generate_labels_single(dset,ae):
    print('Generating mids and latents...')
    info_dict = generate_mids_and_latents(dset,ae.enc)
    mids,latents = info_dict['mids'], info_dict['latents']
    print('Umapping mids...')
...
def label_single(ae_output):
    mids,latents = ae_output['mids'],ae_output['latent']
    print('Umapping mids...')
    umapped_mids = umap.UMAP(min_dist=0,n_neighbors=30,random_state=42).fit_transform(mids.squeeze())
    print('Umapping latents...')
    umapped_latents = umap.UMAP(min_dist=0,n_neighbors=30,random_state=42).fit_transform(latents.squeeze())
    ...
    np.save(f'../labels/mid_labels{ae.identifier}.npy',mid_labels)
    np.save(f'../labels/latent_labels{ae.identifier}.npy',latent_labels)
    return {'aeid':ae_output[aeid],'mid_labels':mid_labels,'latent_labels':latent_labels}

def generate_vecs_single(dset,ae):
    ...
    for i, (xb,yb,idx) in enumerate(determin_dl):
        ...
        mids = mid.view(mid.shape[0],-1).detach().cpu() if i==0 else torch.cat([total_mids,mid.view(mid.shape[0],-1).detach().cpu()],dim=0)
        latents = latent.view(latent.shape[0],-1).detach().cpu() if i==0 else torch.cat([total_latents,latent.view(latent.shape[0],-1).detach().cpu()],dim=0)
    np.save(f'../vecs/mids{ae.identifier}.npy',mids)
    np.save(f'../vecs/latents{ae.identifier}.npy',latents)
    return {'aeid':ae.identifier,'mids':mids,'latents':latents}
    
#def load_aes(aeids): return aes_by_id = {aeid: np.load(f'../checkpoints/{aeid}.pt') for aeid in aeids}
def load_ae(aeid): 
    chkpt = torch.load(f'../checkpoints/{aeid}.pt')
    revived_ae = AE(chktp['enc'],chkpt['dec'],aeid)
    ...
    with ctx.Pool(processes=4) as pool:
        ...
        if 1 in ARGS.sections: #Pretrain aes rather than load
            ...
            for i in range(ARGS.num_aes):
                ...
                dec.to('cuda')
                aes.append(utils.AE(enc,dec,i))
            filled_pretrain = partial(pretrain_ae,args=ARGS,dset=DATASET)
            aes = map(filled_pretrain,aes) if ARGS.single else pool.map(filled_pretrain, aes)
        elif 3 in ARGS.sections or 5 in ARGS.sections:
            aes = map(load_ae,aeids) if ARGS.single else pool.map(load_ae, aeids)

        if 2 in ARGS.sections:
            filled_generate = partial(generate_vecs_single,dset=DATASET)
            vecs = map(filled_generate,aes) if ARGS.single else pool.map(filled_generate, aes)
        elif 4 in ARGS.sections:
            vecs = map(load_ae,aeids) if ARGS.single else pool.map(load_ae, aeids)

        if 3 in ARGS.sections:
            labels = map(label_single,vecs) if ARGS.single else pool.map(label_single, vecs)
        elif 4 in ARGS.sections:
            labels = map(load_labels_single,aeids) if ARGS.single else pool.map(load_labels_single,aeids)

        if 4 in ARGS.sections:
            vecs_and_labels = {aeid:{**vecs[aeid],**labels[aeid]} for aeid in vecs.keys()}
        ...
        if 5 in ARGS.sections:
            filled_train = partial(train_ae,args=ARGS,dset=DATASET,centroids_by_id=centroids_by_id,ensemble_labels=ensemble_labels,all_agree=all_agree)
            map(filled_train,aes) if ARGS.single else pool.map(filled_train, aes)
