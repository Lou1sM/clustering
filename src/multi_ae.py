...
def rtrain_ae(ae_dset_dict,args,should_change):
    #os.environ['CUDA_VISIBLE_DEVICES'] = str(ae_dset_dict['dset'].device.index)
    with torch.cuda.device(ae_dset_dict['dset'].device):
        model_id,ae,dset = ae_dset_dict['model_id'],ae_dset_dict['model'],ae_dset_dict['dset']
        ...
        print(f'Rtraining {model_id} on {dset.device}')
        ...
        for epoch in range(args.pretrain_epochs):
            ...
            for i,(xb,yb,idx) in enumerate(dl):
                ...
                if args.test: break
            print(f'Rtraining AE {model_id}, Epoch: {epoch}, Loss {round(total_loss,4)}')
            if args.test: break
        ...
        if args.save:
            utils.torch_save({'model':ae}, f'../{args.dset}/checkpoints',f'pretrained{model_id}.pt')
        ...
        print('finished', model_id)
        return {'model_id':model_id,'latents':latents}
...
def rtrain_trans(trans_dict,args,dset,should_change):
    ...
    filled_jagged_collate = functools.partial(jagged_collate,device=transformer.device)
    ...
    return {'model_id':model_id,'latents':all_pooled_latents}

def label_single(output,args,abl=False):
    model_id,all_pooled_latents = output['model_id'],output['latents']
    if args.test:
...
def train_ae(ae_dset_dict,args,targets,all_agree,dset,sharing_ablation):
    with torch.cuda.device(ae_dset_dict['dset'].device):
        model_id,ae,dset = ae_dset_dict['model_id'],ae_dset_dict['model'],ae_dset_dict['dset']
        dl = data.DataLoader(dset,batch_sampler=data.BatchSampler(data.RandomSampler(dset),args.batch_size,drop_last=True),pin_memory=False)
        ...
        if sharing_ablation:
            targets_arr = np.array(targets[model_id]['labels'])
        ...
        for epoch in range(args.epochs):
            ...
            for i, (xb,yb,idx) in enumerate(dl):
                ...
                loss.backward(); opt.step(); opt.zero_grad()
                if args.test: break
            print(f'AE: {model_id}, Epoch: {epoch} RLoss: {round(total_rloss,3)}, GaussLoss: {round(total_gloss,3)}')
            if args.test: break
        ...
        if args.save:
            utils.torch_save({'enc':ae.enc,'dec':ae.dec}, f'../{args.dset}/checkpoints/{args.exp_name}',f'{model_id}.pt')
        ...
        return {'model_id':model_id,'latents':latents,'direct_labels':direct_labels}
...
def train_transformer(transformer_dict,args,targets,all_agree,dset):
    ...
    if args.test:
        ...
        for i, (hiddens,lengths,idx) in enumerate(determin_dl):
            ...
            pooled_latents = pooled_latents.detach().cpu().numpy()
            all_pooled_latents = pooled_latents if i==0 else np.concatenate([all_pooled_latents,pooled_latents],axis=0)
    return {'model_id':model_id,'direct_labels':direct_labels,'latents':all_pooled_latents}

def load_model(model_dict_without_model,args):
    model_id, dset = model_dict_without_model['model_id'], model_dict_without_model['dset']
    ...
    #return dict(model=revived_model, **model_dict_without_model)
    return dict(model=revived_model)

def load_vecs(model_id,args):
    latents = np.load(f'../{args.dset}/vecs/latents{model_id}.npy')
    return {'model_id':model_id, 'latents':latents}
