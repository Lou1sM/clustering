...
def build_ensemble(vecs_and_labels):
    ...
    centroids_by_id = {}
    for aeid in set(usable_mid_labels.keys()).union(set(usable_latent_labels.keys())):
...
def train_ae(ae_dict,args,centroids_by_id,ensemble_labels,all_agree):
    ...
    with torch.cuda.device(device):
        ...
        try: mid_centroids = torch.tensor(centroids_dict['mid_centroids'],device='cuda')
        except KeyError: print(f"Can't train ae {aeid}, no mids"); return
        try: latent_centroids = torch.tensor(centroids_by_id[aeid]['latent_centroids'],device='cuda')
        except KeyError: print(f"Can't train ae {aeid}, no latents"); return
        ...
        try:
            assert len(gt_labels.unique()) == mid_centroids.shape[0]
            assert len(gt_labels.unique()) == latent_centroids.shape[0]
        except: set_trace()
        for epoch in range(args.epochs):
            ...
            for i, (xb,yb,idx) in enumerate(dl):
                try:
                    ...
                    loss.backward(); opt.step(); opt.zero_grad()
                except: print(epoch,i,'aeid',aeid)
                if args.test: break
            print(f'R Loss {total_rloss}, C Loss: {total_closs}, CR Loss: {total_crloss}')
            if args.test: break
...
def load_ensemble(aeids):
    ...
    for aeid in aeids:
        ...
        try: new_centroids['mid_centroids'] = np.load(f'mid_centroids{aeid}.npy')
        except FileNotFoundError: pass
        try: new_centroids['latent_centroids'] = np.load(f'latent_centroids{aeid}.npy')
        except FileNotFoundError: pass
...
if __name__ == "__main__":
    ...
    group.add_argument('--num_aes',type=int,default=10)
    ...
    aeids = ARGS.aeids if ARGS.aeids is not None else range(ARGS.num_aes)
    with ctx.Pool(processes=20) as pool:
        ...
        if 5 in ARGS.sections:
            aeids = centroids_by_id.keys()
            aes = [ae for ae in aes if ae['aeid'] in aeids]
